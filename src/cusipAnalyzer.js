// cusipAnalyzer.js - CUSIP Analysis Module
...
export async function analyzeCusip(cusip, settlementDateStr, issuePreference = 'latest', env) {
    try {
        ...
        return {
            ...
            selected_issue: {
                which: issuePreference === 'original' ? 'Original Issue' : 'Latest Issue',
                issue_date: selectedIssue.issueDate,
                auction_date: selectedIssue.auctionDate,
                reopening: selectedIssue.reopening,
                bid_to_cover_ratio: selectedIssue.bidToCoverRatio
            },
            settlement_info: {
                today: today.toISOString().split('T')[0],
                settlement_date: settlementDate.toISOString().split('T')[0],
                specified: !!settlementDateStr
            },
            price_info: {
                cusip: price.cusip,
                security_type: price.securityType,
                security_term: price.securityTerm,
                maturity_date: price.maturityDate,
                issue_date: selectedIssue.issueDate,
                coupon_rate: price.couponRate,
                payment_frequency: price.interestPaymentFrequency,
...
function validateCUSIP(cusip) {
    ...
    // Next 3 can be letters or digits
